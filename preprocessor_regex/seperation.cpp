...
{
	ifstream infile;
	infile.open("l7-reduced.re",ifstream::in);
	string temp;
...
{
	ifstream infile;
	infile.open("substrings.txt",ifstream::in);
	ofstream outfile("dotstar.txt",ios::out | ios::trunc);
	vector <string> data;
	...
	{
		temp.clear();
		for(int j=0;j<NO_STAR;j++)
		{
			it = data.begin() + rand()%177;
			while((*it)[0] == '^')
				it = data.begin() + rand()%177;
			temp.append(*it);
		 	if(j != NO_STAR - 1)
